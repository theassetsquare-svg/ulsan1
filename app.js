...
/* 페이지별 정적 HTML 분리 이후: 각 URL은 자기 article만 들고 있다.
   DOM에 없는 페이지는 SPA로 못 그리므로 실제 주소로 이동시킨다. 결코 메인으로 되돌리지 않는다. */
...
var SEARCH_DATA=[
  ...
  {page:'nearbya',icon:'🍻',name:'주변 코스',keywords:'울산챔피언나이트 주변 코스 1차 다음 자리 고기 삼겹살 해장 국밥 맛집 동선'},
...
var ttTypes=[
  ...
  {name:'🧠 전략가형',icon:'🧠',desc:'가기 전부터 계획이 다 있다. 몇 시 출발, 뭐 마실지, 대리비 얼마, 다음 자리 어디. 엑셀 안 쓴 게 신기할 정도다. 이런 사람이 팀에 한 명은 있어야 밤이 망하지 않는다. 근데 가끔은 계획 없이 가봐. 예상 못 한 밤이 제일 기억에 남더라.'},
...
function shareTTResult(name){
  if(navigator.share){
    navigator.share({title:'나의 나이트 유형: '+name,text:'울산챔피언나이트 유형 테스트 결과: '+name+' 🎉',url:'https://e.nolcool.com/'});
  }else{
    navigator.clipboard.writeText('나의 나이트 유형: '+name+' 🎉 https://e.nolcool.com/').then(function(){showToast('📋 복사됐다. 친구한테 보내봐')});
  }
}
