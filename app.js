...
function go(page){
  try{
    ...
    if(el){
      ...
      var imgUrl=og?'https://ulsan1.pages.dev/'+og:'https://ulsan1.pages.dev/og/main.png';
      var pageUrl=page?'https://ulsan1.pages.dev/'+page:'https://ulsan1.pages.dev/';
      /* OG tags */
...
function shareTTResult(name){
  if(navigator.share){
    navigator.share({title:'나의 나이트 유형: '+name,text:'울산챔피언나이트 유형 테스트 결과: '+name+' 🎉',url:'https://ulsan1.pages.dev/'});
  }else{
    navigator.clipboard.writeText('나의 나이트 유형: '+name+' 🎉 https://ulsan1.pages.dev/').then(function(){showToast('📋 복사됐다. 친구한테 보내봐')});
  }
}
